...
void PhysicsEngine:: explosion(btVector3& point)
{
	Ogre::Vector3 tempPoint (point.getX(), point.getY(), point.getZ()); 
	
	btVector3 temp; 

	btScalar tempX = temp.getX(); 
	btScalar tempY = temp.getY(); 
	btScalar tempZ = temp.getZ(); 

	

	for (int i = -150; i < 150 ; i += 20) 
	{
		for (int j = -150; j < 150 ; j += 20) 
		{
			for (int k = -150; k < 150 ; k += 20)
			{
				tempX = i; 
				tempY = j; 
				tempZ = k; 

				Ogre::Vector3 tempDes (i, j , k); 
				temp.setValue (tempX, tempY, tempZ); 

				Ogre::Ray r (tempPoint, tempDes); 
				r.setOrigin (tempPoint); 
		
				temp.setValue (tempX, tempY, tempZ); 		
				btVector3 linearVelocity(temp - point);		
				linearVelocity.normalize ();				
				linearVelocity *= 3.0f;

				//shootRay (point, temp, linearVelocity);
				shootRay (convert(r.getOrigin()), convert(r.getPoint(50)), linearVelocity);
			}
		}
	}
